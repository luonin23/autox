...
public class RuntimeAgent {
    ...
    private String systemPrompt() {
        return "你是 Fold7 Agent 的运行时页面导航 Agent。你不是一次性写脚本，而是在每一步根据当前手机页面决定下一步动作。返回严格 JSON，不要输出多余文字。格式：{\"status\":\"continue|done|error\",\"reason\":\"...\",\"action\":{\"action\":\"动作名\", \"text\":\"...\"}}。action 必须是对象，且对象内必须包含 action 字段。可用 action：open_app(package/appName), open_settings, tap_text(text,timeoutMs), tap_xy(x,y), input_text(text,timeoutMs), scroll_down, scroll_up, wait(ms), back, home。优先 tap_text/input_text/scroll_down/scroll_up/back/home。input_text 是原生无障碍直接写入文本，不需要切换键盘、中英模式或逐字点击键盘。需要滚动列表时必须使用 scroll_down 或 scroll_up，禁止用 tap_xy 代替滚动。只有没有稳定文本节点时才使用 tap_xy。tap_xy 的 x/y 必须使用当前截图的真实屏幕像素坐标，也可以使用 0-1 归一化坐标；不要使用缩略图坐标。页面不在目标位置时，先用 back 或 home 回到稳定入口，再继续。找不到元素时不要假装成功，返回 error 或选择合理的恢复动作。";
    }
    ...
    private String userPrompt(String request, JSONObject plan, String observation, String history, String lastAction, boolean hasImage) {
        return "用户原始目标：\n" + request
            + "\n\n高层动作计划：\n" + plan.toString()
            + "\n\n当前页面观察：\n" + observation
            + "\n\n截图：\n" + (hasImage ? "已附加当前手机原始尺寸截图，请优先根据截图判断页面状态。若需要 tap_xy，请输出截图真实像素坐标，或输出 0-1 归一化坐标。" : "当前无法截图，请根据页面观察判断。")
            + "\n\n已执行历史：\n" + history
            + "\n\n上一步动作或失败：\n" + lastAction
